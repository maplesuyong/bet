...
router.get('/profile/bet/:bet_id', function(req, res){
    if(req.session.user){
        db.connect(function(err) {
            db.query('SELECT * FROM bets WHERE user_id = ? AND bet_id = ?', [req.session.user.code, req.params.bet_id], function(err,result){
                ...
                db.query(query, function (err, result2) {
                    if (err) throw err;
                    res.render("profile_bet_detail", {'result' : result, 'result2' : result2});
                });
            });
        });
    } else {
...
router.post('/charge_apply', function(req, res){
    var datetime = new Date();
    if(req.session.user){
        db.connect(function(err) {
